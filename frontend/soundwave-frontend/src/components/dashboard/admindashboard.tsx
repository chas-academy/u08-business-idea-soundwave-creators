...
////updated with api 
const fetchUsers = () => {
    axios.get(`${API.defaults.baseURL}/users/users`)
        .then(response => {
            if (!response.data) {
...
const fetchSongs = () => {
    axios.get(`${API.defaults.baseURL}/songs/songs`)
        .then(response => {
            if (!response.data) {
                throw new Error('No data received');
            }
...
//delete songs
    const handleDeleteSong = (id: string) => {
        axios.delete(`${API.defaults.baseURL}/songs/${id}`)
            .then(() => {
                fetchSongs();
            })
    ...
    const handleDeleteUser = (id: number) => {
        axios.delete(`${API.defaults.baseURL}/users/${id}`)
            .then(() => {
                fetchUsers();
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            case 'tab1':
                ...
                {users.map(user => (
                <div key={user._id} className="flex justify-between p-2 bg-gray-700">
            ...
            case 'tab2':
                return (
                    <div>
                        {songs.map(song => (
                            <div key={song._id} className="flex justify-between p-2 bg-gray-700">
            ...
            <section className="pl-6 pr-6 sm:pl-10 sm:pl-28 sm:pr-28  max-h-screen:full">
                ...
                <div className="rounded-2xl shadow-secondary grid grid-cols-3 mt-3 gap-4 pt-8 pb-8 sm:gap-6 ">
                    <div className="bg-secondary p-5 rounded-lg shadow-lg flex flex-col items-center">
                        <FontAwesomeIcon icon={faUserPlus} className="sm:text-3xl mb-1 sm:mt-8 sm:mb-6 " />
                        <h1>{users.length}</h1>
                    ...
                    <div className="bg-secondary p-5 rounded-lg shadow-lg flex flex-col items-center">
                        <FontAwesomeIcon icon={faThumbsUp} className="sm:text-3xl mb-1 sm:mt-8 sm:mb-2 " />
                        <h1>{songs.length}</h1>
                 ...
                 <div className="tabs border shadow-secondary border-secondary mt-5 p-3 sm:p-8 rounded-2xl sd:font-xs">
                    <ul className="flex space-x-4 mb-4 sd:text-s">
                        <li className={`cursor-pointer ${activeTab === 'tab1' ? 'text-yellow-300' : 'text-gray-400'}`} onClick={() => {setActiveTab('tab1'); fetchUsers();}}>All Users</li>
                        <li className={`cursor-pointer ${activeTab === 'tab2' ? 'text-yellow-300' : 'text-gray-400'}`} onClick={() => {setActiveTab('tab2'); fetchSongs();}}>Songs</li>
                        <li className={`cursor-pointer ${activeTab === 'tab3' ? 'text-yellow-300' : 'text-gray-400'}`} onClick={() => setActiveTab('tab3')}>Subscriptions</li>
                    </ul>
